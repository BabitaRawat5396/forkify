...
async function fetchData(inputValue) {
    try {
        // ignore-prettier
        const response = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes?search=${inputValue}`);
...
function renderSearchedResults() {
    ...
    `;
        ...
        document.querySelector(".pagination-forward").addEventListener("click", function() {
    ...
    if (count > 9) {
        ...
        document.querySelector(".pagination-backward").addEventListener("click", function() {
            if (count <= 0) return;
            count -= 10;
            renderSearchedResults();
        });
    ...
    <div class="recipe">
      ...
      <div class="recipe-operations-section">
        ...
        <div class="recipe-servings">
          ...
          <p class="servings-no"><span class="numbers">${recipe.servings}</span> SERVINGS</p>
...
window.addEventListener("hashchange", async function(e) {
    ...
    const recipeContent = await fetch(`https://forkify-api.herokuapp.com/api/v2/recipes/${id}`);
